...
class AbstractBacklash:
        ...
        if (len(self._parameters_labels) != len(self._parameters_units)) \
           or (len(self._parameters_labels) != self.parameters_count) :
               raise Exception("Invalid parameters passed to __init__().\
                            The labels and units list should be equal in length (and equal to parameters_count) !")
    ...
    def clear_dynamic_properties(self) :
        ...
        Note
        -------
        Can be called in child class as 'super().clear_dynamic_properties()' to make sure obsolete data is not kept.
        '''
        # Flag all properties as invalid
        self._active_branch = None
    ...
    def evaluate(self, x_k_1 : np.ndarray, q_k : np.ndarray, q_k_1  : np.ndarray = None) -> np.ndarray:
        '''
        Evaluate the state of the backlash element and returns True if successful.
